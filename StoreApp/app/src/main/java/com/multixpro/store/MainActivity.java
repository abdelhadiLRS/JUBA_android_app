...
public class MainActivity extends ParentActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        orderCountTxt = findViewById(R.id.orderCountTxt);
        ...
        addToClickHandler(earningArea);
        addToClickHandler(SettingArea);
        ...
        if (eStatus.equalsIgnoreCase("inactive")) {
            ...
            if (generalFunc.getJsonValueStr("eAutoaccept", obj_userProfile) != null && generalFunc.getJsonValueStr("eAutoaccept", obj_userProfile).equalsIgnoreCase("Yes")) {
...
//        }
    ...
    public void openPageFrag(int position, Fragment fragToOpen) {
        ...
        getSupportFragmentManager().beginTransaction().setCustomAnimations(leftAnim, rightAnim)
                .replace(R.id.container1, fragToOpen).commit();
    ...
    public void manageBottomMenu(MTextView selTextView) {
        ...
        if (selTextView.getId() == earnIngTxt.getId()) {
            ...
            earnImg.setImageResource(R.drawable.ic_earning_fill);
        } else {
            ...
            earnImg.setImageResource(R.drawable.ic_earning);
        }
        ...
        if (selTextView.getId() == settingTxt.getId()) {
            ...
            settingImg.setImageResource(R.drawable.ic_item_fill);
        } else {
            ...
            settingImg.setImageResource(R.drawable.ic_item);
    ...
    public void resetNames(int size, int size1, int size2, String orderType) {
        ...
        /*boolean isCurrentShow = fragmentList.get(viewPager.getCurrentItem()).getArguments().getString("ORDER_TYPE").equalsIgnoreCase(orderType);
        orderCountTxt.setVisibility(View.VISIBLE);

        if (viewPager.getCurrentItem() == 0 && isCurrentShow) {
            orderCountTxt.setText(generalFunc.retrieveLangLBl("YOU HAVE", "LBL_ORDERS_TITLE_COUNT_PREFIX") + " " + generalFunc.convertNumberWithRTL(size + " ") + generalFunc.retrieveLangLBl("new order", "LBL_ORDERS_TITLE_COUNT_POSTFIX_NEW") + ((size < 1) ? "" : "(" + generalFunc.retrieveLangLBl("S", "LBL_S_POSTFIX") + ")"));
        } else if (viewPager.getCurrentItem() == 1 && isCurrentShow) {
            orderCountTxt.setText(generalFunc.retrieveLangLBl("YOU HAVE", "LBL_ORDERS_TITLE_COUNT_PREFIX") + " " + generalFunc.convertNumberWithRTL(size1 + " ") + generalFunc.retrieveLangLBl("in process order", "LBL_ORDERS_TITLE_COUNT_POSTFIX_IN_PROCESS") + ((size1 < 1) ? "" : "(" + generalFunc.retrieveLangLBl("S", "LBL_S_POSTFIX") + ")"));
        } else if (viewPager.getCurrentItem() == 2 && isCurrentShow) {
            orderCountTxt.setText(generalFunc.retrieveLangLBl("YOU HAVE", "LBL_ORDERS_TITLE_COUNT_PREFIX") + " " + generalFunc.convertNumberWithRTL(size2 + " ") + generalFunc.retrieveLangLBl("in process order", "LBL_ORDERS_TITLE_COUNT_POSTFIX_DISPATCHED") + ((size2 < 1) ? "" : "(" + generalFunc.retrieveLangLBl("S", "LBL_S_POSTFIX") + ")"));
        }
    ...
    public void newOrderReceived(boolean isKiosk) {
        Logger.d("newOrderReceived", "::" + viewPager.getCurrentItem());


        if (ordersPagerAdapter.getCount() > 0 && viewPager.getCurrentItem() == 0) {
            ...
            if ((generalFunc.getJsonValueStr("eAutoaccept", obj_userProfile) != null && generalFunc.getJsonValueStr("eAutoaccept", obj_userProfile).equalsIgnoreCase("Yes")) || isKiosk) {
                viewPager.setCurrentItem(1);
                manageAutoaccept(isKiosk);
    ...
    public void manageAutoaccept(boolean isKiosk) {
        if ((generalFunc.getJsonValueStr("eAutoaccept", obj_userProfile) != null && generalFunc.getJsonValueStr("eAutoaccept", obj_userProfile).equalsIgnoreCase("Yes")) || isKiosk) {


            if (ordersPagerAdapter.getCount() > 0 && ordersPagerAdapter.getItem(1) instanceof OrderFragment) {

                if (!MyApp.getInstance().isMyAppInBackGround()) {
                    Utils.vibratePhone(getActContext(), 1000);
                    Utils.playNotificationSound(getActContext());
                }
                if (((OrderFragment) ordersPagerAdapter.getItem(1)).newOrderNotificationArea != null) {
                    ((OrderFragment) ordersPagerAdapter.getItem(1)).newOrderNotificationArea.setVisibility(View.VISIBLE);
                    ((OrderFragment) ordersPagerAdapter.getItem(1)).orderRecyclerAdapter.hilightNewOrder();
                }


                if (newautoOrderHandler != null) {
                    newautoOrderHandler.removeCallbacks(newautoOrderRunnable);
                }
                newautoOrderHandler = new Handler();

                newautoOrderHandler.postDelayed(newautoOrderRunnable, 6000);
...
//                bn.putString("msg", "DO_PHONE_VERIFY");
    ...
    public void openMenuProfile() {
        new ActUtils(getActContext()).startActForResult(MyProfileActivity.class, Utils.MY_PROFILE_REQ_CODE);
    }
...
//                (new ActUtils(getActContext())).startActForResult(ThermalPrintSettingActivity.class, Utils.SETTINGS_UPDATED_CODE);
//            }
...
//            }
    ...
    Runnable newautoOrderRunnable = () -> {
        ((OrderFragment) ordersPagerAdapter.getItem(1)).orderRecyclerAdapter.delightNewOrder();
        ((OrderFragment) ordersPagerAdapter.getItem(1)).newOrderNotificationArea.setVisibility(View.GONE);
    };
}
