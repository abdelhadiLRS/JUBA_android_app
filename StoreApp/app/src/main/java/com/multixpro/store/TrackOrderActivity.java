...
//        getTrackDetails();
    ...
    public void pubnubMessage(JSONObject obj_msg) {
        ...
        if (!messageStr.equals("")) {
            ...
            } else if (messageStr.equalsIgnoreCase("OrderDeclineByRestaurant") || messageStr.equalsIgnoreCase("OrderDelivered")
...
//                if (generalFunc.getJsonValue("iDriverId", obj_msg) != null && !generalFunc.getJsonValue("iDriverId", obj_msg).equals("")) {
//                    iDriverId = generalFunc.getJsonValue("iDriverId", obj_msg).toString();
                    ...
                    responseString -> {
                        ...
                        if (responseString != null && !responseString.equals("")) {
                            ...
                            if (action.equals("1")) {
                                ...
                                generateMapLocations(generalFunc.parseDoubleValue(0, generalFunc.getJsonValue("CompanyLat", responseString)), generalFunc.parseDoubleValue(0, generalFunc.getJsonValue("CompanyLong", responseString)), generalFunc.parseDoubleValue(0, generalFunc.getJsonValue("PassengerLat", responseString)), generalFunc.parseDoubleValue(0, generalFunc.getJsonValue("PassengerLong", responseString)));
                                ...
                                titleTxt.setText(generalFunc.retrieveLangLBl("", "LBL_ORDER") + " #" + generalFunc.convertNumberWithRTL(generalFunc.getJsonValue("vOrderNo", responseString)));
//                        subTitleTxt.setText(generalFunc.convertNumberWithRTL(generalFunc.getDateFormatedType(generalFunc.getJsonValue("tOrderRequestDate", responseString), Utils.OriginalDateFormate, Utils.TrackDateFormatewithTime)));

                                String tOrderRequestDate = generalFunc.getJsonValue("tOrderRequestDate", responseString);

                                String formattedDate = generalFunc.getDateFormatedType(tOrderRequestDate, Utils.OriginalDateFormate, CommonUtilities.OriginalDateFormate) + ", " + generalFunc.getDateFormatedType(tOrderRequestDate, Utils.OriginalDateFormate, CommonUtilities.OriginalTimeFormate);
                                subTitleTxt.setText(generalFunc.convertNumberWithRTL(formattedDate));

                                JSONArray messageArr = null;
                                messageArr = generalFunc.getJsonArray(Utils.message_str, responseString);
    ...
    public void validateIncomingOrderMessage(String message) {
        ...
        String iOrderId = generalFunc.getJsonValue("iOrderId", message);
        if (iOrderId.equalsIgnoreCase(getIntent().getStringExtra("iOrderId"))) {
//            getTrackDetails();
            pubnubMessage(generalFunc.getJsonObject(message));
        }
    }

    public void validateIncomingMessage(String message) {

        String iDriverId_tmp = generalFunc.getJsonValue("iDriverId", message);
