...
import com.multixpro.store.SearchLocationActivity;
...
public class OpenNoLocationView {
    ...
    public void configView(boolean isLocationEnabled) {
        if (viewGroup != null && currentAct != null) {
            ...
            if (currentAct instanceof TrackOrderActivity || (currentAct instanceof MainActivity && MyApp.getInstance().trackOrderAct != null)) {
            ...
            noLocMenuImgView.setOnClickListener(v -> {
                if (currentAct instanceof MainActivity && MyApp.getInstance().trackOrderAct == null) {
            ...
            settingsBtn.setOnClickListener(v -> {
                if (!generalFunc.isLocationPermissionGranted(false)) {
                    int myCount = 0;
                    String[] mPermissions = new String[]{android.Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION};
                    for (String permission : mPermissions) {
                        if (!ActivityCompat.shouldShowRequestPermissionRationale(currentAct, permission)) {
                            myCount++;
                        }
                    }
            ...
            if (!generalFunc.isLocationPermissionGranted(false)) {
                ...
                noLocMsgTxt.setText(generalFunc.retrieveLangLBl("", "LBL_LOCATION_PERMISSION_DENIED_DETAILS"));
