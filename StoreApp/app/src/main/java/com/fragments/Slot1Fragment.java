                            ...
                            if (generalFunc.checkDataAvail(Utils.action_str, responseString) == true) {
                                ...
                                if (callType.equalsIgnoreCase("Display")) {
                                    ...
                                    for (int j = 0; j < msg_obj.length(); j++) {
                                        ...
                                        JSONArray obj_date_temp = generalFunc.getJsonArray("" + (j + 1), msg_obj.toString());
                                        for (int k = 0; k < obj_date_temp.length(); k++) {
                                            try {
                                                JSONObject jsonObject = obj_date_temp.getJSONObject(k);
                                                HashMap<String, String> map = new HashMap<String, String>();
                                                map.put("Slot", "" + (j + 1));
                                                map.put("SlotName", (k == 0) ? ("Slot " + (j + 1)) : "");
                                                map.put("SlotName", (k == 0) ? ((j == 0) ? generalFunc.retrieveLangLBl("", "LBL_SLOT_1") : generalFunc.retrieveLangLBl("", "LBL_SLOT_2")) : "");
                                                map.put("isMandatory", (j == 0) ? "Yes" : "No");
//                                    map.put("isMandatory", "No");
                                                String field = generalFunc.getJsonValue("field", jsonObject.toString());
                                                String fromSlotName = ("v" + field + "FromSlot" + "" + (j + 1));
                                                String toSlotName = ("v" + field + "ToSlot" + "" + (j + 1));
                                    ...
                                    adapter = new RestaurantTimeSlotAdapter(Slot1Fragment.this.getActContext(), timeSlotList, generalFunc, Slot1Fragment.this.getActivity().getSupportFragmentManager());
                                    timeslotsRecyclerView.setAdapter(adapter);
