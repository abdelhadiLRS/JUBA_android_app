...
import com.multixpro.store.NotificationDetailsActivity;
...
//        notificationRecyclerView.addItemDecoration(new DividerItemDecoration(getActContext(), DividerItemDecoration.VERTICAL_LIST));
    ...
    public void getNotificationDetails(boolean isLoadMore) {
        ...
        ApiHandler.execute(getActContext(), parameters, isLoadMore ? false : true, false, generalFunc,
                responseString -> {
                    ...
                    if (responseObj != null && !responseObj.equals("")) {
                        if (GeneralFunctions.checkDataAvail(Utils.action_str, responseObj)) {
                            ...
                            JSONArray arr_notifications = generalFunc.getJsonArray(Utils.message_str, responseObj);
                            ...
                            if (list.size() == 0) {
                                removeNextPageConfig();
                                noDataTxt.setText(generalFunc.retrieveLangLBl("", generalFunc.getJsonValueStr(Utils.message_str, responseObj)));
