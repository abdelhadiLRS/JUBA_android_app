    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        ...
        if (holder instanceof ViewHolder) {
            ...
            if (driverImageName == null || driverImageName.equals("") || driverImageName.equals("NONE")) {
                ...
                new LoadImage.builder(LoadImage.bind(image_url), viewHolder.userProfileImgView).setErrorImagePath(R.mipmap.ic_no_pic_user).setPlaceholderImagePath(R.mipmap.ic_no_pic_user).build();
            }

            viewHolder.ratingBar.setRating(generalFunc.parseFloatValue(0, item.get("vRating1")));
            viewHolder.dateTxt.setText(generalFunc.convertNumberWithRTL(generalFunc.convertNumberWithRTL(generalFunc.getDateFormatedType(item.get("tDateOrig"), Utils.OriginalDateFormate, CommonUtilities.OriginalDateFormate))));
