...
public class UpdateDriverLocationService extends Service implements RecurringTask.OnTaskRunCalled, GetLocationUpdates.LocationUpdatesListener {
    ...
    public void updateDriverLocations() {
        ...
        this.currentExeTask = ApiHandler.execute(getApplicationContext(), parameters, responseString -> Logger.d("Api", "Update Locations Response ::" + responseString));
...
//                ConfigPubNub.getInstance().publishMsg(generalFunc.getLocationUpdateChannel(), generalFunc.buildLocationJson(driverLocation, "LocationUpdateOnTrip"));
                ...
                AppService.getInstance().executeService(new EventInformation.EventInformationBuilder().setChanelList(channelName).setMessage(generalFunc.buildLocationJson(driverLocation, "LocationUpdateOnTrip")).build(), AppService.Event.PUBLISH);
            }
        }
    }
