...
public class LocationUpdateService extends Service implements UpdateDriverLocation.OnDemoLocationListener {
    ...
    private void startLocationUpdateService() {
        ...
        if (MyApp.getInstance().locationPermissionReq(false)) {
            ...
            if (getLastLocation() != null) {
                continueDispatchingLocation(getLastLocation());
            }
    ...
    protected void showAppBadgeFloat(Intent intent) {
        ...
        if (!generalFunc.canDrawOverlayViews(mContext) || (!Utils.checkText(iTripId) && generalFunc.retrieveValue(Utils.DRIVER_ONLINE_KEY).equals("false"))) {
            return;
        }
        ...
        if (MyApp.getInstance().getCurrentAct() != null) {
            MyApp.getInstance().getCurrentAct().runOnUiThread(() -> mWindowManager.addView(mAppBgHeadView, params));
        }
