...
class UpdateDriverLocation implements RecurringTask.OnTaskRunCalled {
    ...
    public void executeProcess() {
        ...
        LOCATION_ACCURACY_METERS = generalFunc.parseIntegerValue(200, data.get("LOCATION_ACCURACY_METERS"));
    ...
    public void setTripStartValue(boolean isStartLocationStorage, boolean isTripStarted, String iTripId) {
        ...
        if (isTripStarted && isStartLocationStorage) {
            ...
            updateTripLocationsTask = new RecurringTask(UPDATE_TIME_INTERVAL);
            updateTripLocationsTask.setTaskRunListener(this);
            updateTripLocationsTask.startRepeatingTask();
        } else if (!isTripStarted && (MyApp.getInstance().mainAct != null || MyApp.getInstance().main22Act != null)) {
...
//                ConfigPubNub.getInstance().publishMsg(generalFunc.getLocationUpdateChannel(), isTripStarted ? generalFunc.buildLocationJson(driverLocation, "LocationUpdateOnTrip") : generalFunc.buildLocationJson(driverLocation));
                ...
                AppService.getInstance().executeService(new EventInformation.EventInformationBuilder().setChanelList(channelName).setMessage(isTripStarted ? generalFunc.buildLocationJson(driverLocation, "LocationUpdateOnTrip") : generalFunc.buildLocationJson(driverLocation)).build(), AppService.Event.PUBLISH);
            }
        }
    }
    ...
    public void updateOnlineAvailability(String status) {
        if (isTripStarted || (!generalFunc.isUserLoggedIn() && !Utils.checkText(generalFunc.getMemberId()))) {
            if (updateDriverStatusTask != null) {
                updateDriverStatusTask.stopRepeatingTask();
                updateDriverStatusTask = null;
            }
        ...
        this.currentDriverStatusExeTask = ApiHandler.execute(mContext, parameters, responseString -> {
        });
    ...
    public void updateDriverLocations() {
        ...
        if (listOfTripLocations.size() > 0 && isDataUploading == false) {
           ...
           /* if (this.currentExeTask != null) {
            ...
            this.currentTripLocationExeTask = ApiHandler.execute(mContext, parameters, responseString -> {
                JSONObject responseStringObject = generalFunc.getJsonObject(responseString);
                boolean isDataAvail = generalFunc.checkDataAvail(Utils.action_str, responseStringObject);

                if (isDataAvail == true) {
                    for (int i = 0; i < tempList.size(); i++) {
                        if (listOfTripLocations != null && listOfTripLocations.size() > 0) {
