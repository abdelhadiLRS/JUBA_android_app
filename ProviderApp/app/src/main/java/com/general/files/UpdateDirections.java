...
import com.multixpro.provider.deliverAll.TrackOrderActivity;
...
public class UpdateDirections implements RecurringTask.OnTaskRunCalled {
    ...
    String serverKey, gMapLngCode = "en", eUnit = "KMs";
    ...
    public void updateDirections() {
        ...
        if (userProfileJsonObj != null && (!generalFunctions.getJsonValueStr("ENABLE_DIRECTION_SOURCE_DESTINATION_DRIVER_APP", userProfileJsonObj).equalsIgnoreCase("Yes") || eFly)) {
            if (destinationLocation != null) {
                ...
                } else if (mcontext instanceof WorkingtrekActivity) {
                    if (destinationLocation.getLatitude() > 0) {
                        WorkingtrekActivity workingtrekActivity = (WorkingtrekActivity) mcontext;
                        workingtrekActivity.setTimetext(generalFunctions.formatUpto2Digit(distance) + "", getTimeTxt(lowestTime));
        ...
        if (eTollSkipped.equalsIgnoreCase("Yes")) {
            istollSkip = true;
        }
        ...
        if (activity instanceof MainActivity || activity instanceof MainActivity_22) {
            ...
            //loader Hide
            AppService.getInstance().executeService(mcontext, new DataProvider.DataProviderBuilder(userLocation.getLatitude() + "", userLocation.getLongitude() + "").setDestLatitude(destinationLocation.getLatitude() + "").setDestLongitude(destinationLocation.getLongitude() + "").setWayPoints(new JSONArray()).setTollAccess(istollSkip).build(), AppService.Service.DIRECTION, data -> {
                manageResult(data);
    ...
    public void manageResult(HashMap<String, Object> data) {
        ...
        if (data.get("ROUTES") == null) {
            if (mcontext instanceof MainActivity) {
                MainActivity mainActivity = (MainActivity) mcontext;
                mainActivity.removeEODTripData(true);
            } else if (mcontext instanceof MainActivity_22) {
                MainActivity_22 mainActivity_22 = (MainActivity_22) mcontext;
                mainActivity_22.removeEODTripData(true);
            }
            generalFunctions.showGeneralMessage("", generalFunctions.retrieveLangLBl("", "LBL_DEST_ROUTE_NOT_FOUND"));
            return;
        }
        ...
        if (responseString != null && !responseString.equalsIgnoreCase("") && data.get("DISTANCE") == null) {
            ...
            if (obj_routes != null && obj_routes.length() > 0) {
                ...
                if (mcontext instanceof DriverArrivedActivity) {
                    ...
                    WorkingtrekActivity workingtrekActivity = (WorkingtrekActivity) mcontext;
                    workingtrekActivity.setTimetext(generalFunctions.formatUpto2Digit(distance_final) + "", getTimeTxt((int) (GeneralFunctions.parseDoubleValue(0, time) / 60)));
                } else if (mcontext instanceof TrackOrderActivity) {
                    TrackOrderActivity trackOrderActivity = (TrackOrderActivity) mcontext;
                ...
                if (lineOptions != null) {
                    ...
                    if (mcontext instanceof MainActivity) {
                        ...
                        if (MainActivity_22.mProgressBarEOD != null) {
                            MainActivity_22.mProgressBarEOD.setVisibility(View.GONE);
                        }

                        if (MainActivity_22.slideButtonEOD != null) {
                            MainActivity_22.slideButtonEOD.setVisibility(View.VISIBLE);
                        }
            ...
            if (mcontext instanceof DriverArrivedActivity) {
                ...
                WorkingtrekActivity workingtrekActivity = (WorkingtrekActivity) mcontext;
                workingtrekActivity.setTimetext(generalFunctions.formatUpto2Digit(distance_final) + "", getTimeTxt(duration));
            } else if (mcontext instanceof TrackOrderActivity) {
                TrackOrderActivity trackOrderActivity = (TrackOrderActivity) mcontext;
