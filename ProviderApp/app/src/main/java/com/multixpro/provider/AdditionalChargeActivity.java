...
public class AdditionalChargeActivity extends ParentActivity implements RecurringTask.OnTaskRunCalled {
    ...
    private MTextView tollfeeHTxt, tollfeeCurrancyTxt, tollfeeCurrancyTxt1;
    private MaterialEditText tollfeeVTxt;
    private MTextView otherfeeHTxt, otherfeeCurrancyTxt, otherfeeCurrancyTxt1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (data.containsKey("vChargesDetailDataAvailable") && data.get("vChargesDetailDataAvailable").equalsIgnoreCase("Yes")) {
            ...
            if (updateApprovalStatusTask == null) {
                updateApprovalStatusTask = new RecurringTask(APPROVAL_STATUS_UPDATE_INTERVAL);
    ...
    public void initViews() {
        ...
        isTollEnable = generalFunc.getJsonValueStr("ENABLE_MANUAL_TOLL_FEATURE", obj_userProfile).equalsIgnoreCase("Yes") ? true : false;
        isOtherChargesEnable = generalFunc.getJsonValueStr("ENABLE_OTHER_CHARGES_FEATURE", obj_userProfile).equalsIgnoreCase("Yes") ? true : false;
        ...
        resendSecAfter = GeneralFunctions.parseIntegerValue(30, generalFunc.getJsonValueStr(Utils.VERIFICATION_CODE_RESEND_TIME_IN_SECONDS_KEY, obj_userProfile));
        ...
        if (isUFX && generalFunc.retrieveValue("ENABLE_VERIFY_ADDITIONAL_CHARGE_FLOW").equalsIgnoreCase("Yes")) {
            skipBtnArea.setVisibility(View.GONE);
            midSeparatorLine.setVisibility(View.GONE);
            skipBtn.setOnClickListener(null);
        } else {
            skipBtnArea.setVisibility(View.VISIBLE);
            midSeparatorLine.setVisibility(View.VISIBLE);
    ...
    private void calculateData(String s, MTextView finalvalTxt) {
        try {
            ...
            finalvalTxt.setText(generalFunc.formatNumAsPerCurrency(generalFunc, " " + generalFunc.convertNumberWithRTL(GeneralFunctions.convertDecimalPlaceDisplay(finaltotal)), CurrencySymbol, true));
        ...
        if (finaltotal < 0) {
            if (discount > 0) {
                discountVTxt.setText("" + (discount - finaltotal));
            }
        }
    }
    ...
    public void callTripEnd(boolean isSkip, String finalCode) {
        ...
        ApiHandler.execute(getActContext(), parameters, true, false, generalFunc,
                responseString -> endTripResponse(responseString));
        ...
        if (responseString != null && !responseString.equals("")) {
            ...
            if (isDataAvail) {
               // MyApp.getInstance().restartWithGetDataApp();
                ...
                if (msg_str.equalsIgnoreCase("DO_RESTART")) {
    ...
    public void onClick(View view) {
        ...
        if (i == skipBtn.getId()) {
            ...
            if (additonallist.get(0).equals(0.0) &&
                    additonallist.get(1).equals(0.0) &&
                    additonallist.get(2).equals(0.0)) {
                ...
                alertBox.setBtnClickList(btn_id -> {
                    ...
                    if (btn_id == 1) {
                        ...
                        } else if ((generalFunc.retrieveValue("ENABLE_MANUAL_TOLL_FEATURE").equalsIgnoreCase("Yes") && !isUFX) || generalFunc.retrieveValue("ENABLE_OTHER_CHARGES_FEATURE").equalsIgnoreCase("Yes") && !isUFX) {
    ...
    public void openVerificationDialog() {
        ...
        cancelTxt.setOnClickListener(v -> sendChargeVerificationCode("No"));

        btn_type2.setOnClickListener(v -> {
            String finalCode = Utils.getText(otp_view);
            if (vConfirmationCode.length() == finalCode.length()) {
                boolean isCodeEntered = Utils.checkText(finalCode) && (vConfirmationCode.equalsIgnoreCase(finalCode));
    ...
    private void sendChargeVerificationCode(String eApproveRequestSentByDriver) {
        ...
        ApiHandler.execute(getActContext(), parameters, !eApproveRequestSentByDriver.equalsIgnoreCase("Yes"), false, generalFunc,
    ...
    private void verificationResponse(String responseString) {
        ...
        if (responseString != null && !responseString.equals("")) {
            ...
            if (action.equals("1")) {
                ...
                if (message_str.equals("DO_RESTART") || message_str.equals(Utils.GCM_FAILED_KEY) || message_str.equals(Utils.APNS_FAILED_KEY) || message_str.equals("LBL_SERVER_COMM_ERROR")) {
                    MyApp.getInstance().restartWithGetDataApp();
                    return;
                }
                vConfirmationCode = generalFunc.getJsonValue("vConfirmationCode", responseString);
                String eApproveRequestSentByDriver = generalFunc.getJsonValue("eApproveRequestSentByDriver", responseString);
                ...
                removeBackTaskAndDismissDialog();
                if (Utils.checkText(generalFunc.getJsonValue("vConfirmationCode", responseString))) {
                    vConfirmationCode = generalFunc.getJsonValue("vConfirmationCode", responseString);
                    openVerificationDialog();
                }
                generalFunc.showGeneralMessage("", generalFunc.retrieveLangLBl("", generalFunc.getJsonValue(Utils.message_str, responseString)));
