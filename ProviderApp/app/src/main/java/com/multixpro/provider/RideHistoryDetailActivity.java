...
public class RideHistoryDetailActivity extends ParentActivity implements OnMapReadyCallback {
    ...
    private String tripData, isVideoCall = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        afterServiceArea = findViewById(R.id.afterServiceArea);
        beforeServiceArea = findViewById(R.id.beforeServiceArea);
        tipPluseImage = findViewById(R.id.tipPluseImage);
        cartypeTxt = findViewById(R.id.cartypeTxt);
        vReasonTitleTxt = findViewById(R.id.vReasonTitleTxt);
        viewReqServicesTxtView = findViewById(R.id.viewReqServicesTxtView);
        viewReqServicesArea = findViewById(R.id.viewReqServicesArea);
        loading = findViewById(R.id.loading);
        progresdefault = findViewById(R.id.progresdefault);
        progresdefault.setVisibility(View.VISIBLE);
        errorView = findViewById(R.id.errorView);
        container = findViewById(R.id.container);

        tipHTxt = findViewById(R.id.tipHTxt);
        tipamtTxt = findViewById(R.id.tipamtTxt);
        tipmsgTxt = findViewById(R.id.tipmsgTxt);

        tiparea = findViewById(R.id.tipArea);
        ...
        addToClickHandler(viewReqServicesTxtView);
    }
                            ...
                            if (arr_rides != null && arr_rides.length() > 0) {
                                for (int i = 0; i < arr_rides.length(); i++) {
                                    ...
                                    if (Utils.checkText(paymentDoneByDetail)) {
                                        (findViewById(R.id.recipientTxt)).setVisibility(View.VISIBLE);
    ...
    public void setLabels(String tripData) {
        ...
        if (isVideoCall != null && isVideoCall.equalsIgnoreCase("Yes")) {
            ...
            ((MTextView) findViewById(R.id.pickUpAddressVTxt)).setText(generalFunc.retrieveLangLBl("", "LBL_VIDEO_CONSULT_AT_YOUR_LOC"));
        }
    ...
    public void setData(String tripData) {
        ...
        if (generalFunc.getJsonValue("eChargeViewShow", tripData) != null && generalFunc.getJsonValue("eChargeViewShow", tripData).equalsIgnoreCase("No")) {
            ...
            (findViewById(R.id.rideNoVTxt)).setLayoutParams(txtParam);
            (findViewById(R.id.rideNoHTxt)).setLayoutParams(txtParam);
        ...
        if (generalFunc.getJsonValue("eType", tripData).equals("Deliver")) {
            ...
            (findViewById(R.id.viewSingleDeliveryTitleTxt)).setVisibility(View.VISIBLE);
            (findViewById(R.id.viewSingleDeliveryDetails)).setVisibility(View.VISIBLE);
            (findViewById(R.id.viewDeliveryDetails)).setVisibility(View.GONE);
        ...
        final ImageView driverImageview = findViewById(R.id.driverImgView);
        ...
        if (generalFunc.getJsonValue("eType", tripData).equalsIgnoreCase("UberX") && generalFunc.getJsonValueStr("SERVICE_PROVIDER_FLOW", obj_userProfile).equalsIgnoreCase("Provider")) {
        ...
        if (generalFunc.getJsonValue("eType", tripData).equalsIgnoreCase("UberX") || generalFunc.getJsonValue("eFareType", tripData).equalsIgnoreCase("Fixed")) {
            ...
            if (!TextUtils.isEmpty(generalFunc.getJsonValue("vBeforeImage", tripData))) {
                ...
                displayPic(vBeforeImage, findViewById(R.id.iv_before_img), "before");
            ...
            if (!TextUtils.isEmpty(generalFunc.getJsonValue("vAfterImage", tripData))) {
                ...
                displayPic(vAfterImage, findViewById(R.id.iv_after_img), "after");
            ...
            if (isVideoCall != null && isVideoCall.equalsIgnoreCase("Yes")) {
                ((MTextView) findViewById(R.id.pickUpAddressVTxt)).setText(generalFunc.retrieveLangLBl("", "LBL_VIDEO_CONSULT_AT_YOUR_LOC"));
                (findViewById(R.id.videoImg)).setVisibility(View.VISIBLE);
                (findViewById(R.id.loactionImg)).setVisibility(View.GONE);
            }
    ...
    private void setRecyclerView() {
        ...
        for (int i = 0; i < recipientDetailList.size(); i++) {
            ...
            recipientNoTxt = convertView.findViewById(R.id.recipientNoTxt);
            recipeientNameTxt = convertView.findViewById(R.id.recipeientNameTxt);
            recipeientMobileTxt = convertView.findViewById(R.id.recipeientMobileTxt);
            dropOffVTxt = convertView.findViewById(R.id.dropOffVTxt);
            dropOffHTxt = convertView.findViewById(R.id.dropOffHTxt);
            ricipientSignTxt = convertView.findViewById(R.id.ricipientSignTxt);
            tripStatusLblTxt = convertView.findViewById(R.id.tripStatusLblTxt);
            tripStatusTxt = convertView.findViewById(R.id.tripStatusTxt);
            deliveryDetailsArea = convertView.findViewById(R.id.deliveryDetailsArea);
    ...
    private void setdeliveriesDetails(String vFieldName, String vValue, int checkItemPos, int listSize, int noRecipient, LinearLayout deliveryDetailsArea) {
        ...
        itemTitleTxt = v.findViewById(R.id.itemTitleTxt);
        itemDetailArea = v.findViewById(R.id.itemDetailArea);
        itemValueTxt = v.findViewById(R.id.itemValueTxt);
    ...
    public void showSignatureImage(String Name, String image_url, boolean isSender) {
        ...
        final ProgressBar LoadingProgressBar = (signatureImageDialog.findViewById(R.id.LoadingProgressBar));
        ...
        if (Utils.checkText(image_url)) {
            ...
            new LoadImage.builder(LoadImage.bind(image_url), signatureImageDialog.findViewById(R.id.passengerImgView)).setPicassoListener(new LoadImage.PicassoListener() {
                @Override
                public void onSuccess() {
                    LoadingProgressBar.setVisibility(View.GONE);
                    (signatureImageDialog.findViewById(R.id.passengerImgView)).setVisibility(View.VISIBLE);
                }

                @Override
                public void onError() {
                    LoadingProgressBar.setVisibility(View.VISIBLE);
                    (signatureImageDialog.findViewById(R.id.passengerImgView)).setVisibility(View.GONE);
                }
            }).build();
    ...
    public void displayPic(String image_url, ImageView view, final String imgType) {
        ...
        new LoadImage.builder(LoadImage.bind(image_url), view).setErrorImagePath(R.mipmap.ic_no_icon).setPlaceholderImagePath(R.mipmap.ic_no_icon).setPicassoListener(new LoadImage.PicassoListener() {
            @Override
            public void onSuccess() {
    ...
    public void drawRoute(LatLng pickUpLoc, LatLng destinationLoc) {
        ...
        AppService.getInstance().executeService(getActContext(), new DataProvider.DataProviderBuilder(pickUpLoc.latitude + "", pickUpLoc.longitude + "").setDestLatitude(destinationLoc.latitude + "").setDestLongitude(destinationLoc.longitude + "").setWayPoints(new JSONArray()).build(), AppService.Service.DIRECTION, new AppService.ServiceDelegate() {
            @Override
            public void onResult(HashMap<String, Object> data) {
                ...
                String responseString = data.get("ROUTES").toString();
