...
public class ChatActivity extends ParentActivity {
    ...
    Context mContext;
    EditText input;
    MTextView userNameTxt, catTypeText, tv_booking_no, driverRating;
...
//    SimpleRatingBar ratingBar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        driverImgName = generalFunc.getJsonValueStr("vImage", obj_userProfile);

        data_trip_ada = new HashMap<>();
        if (getIntent().hasExtra("iBiddingPostId")) {
            data_trip_ada.put("iBiddingPostId", getIntent().getStringExtra("iBiddingPostId") + "_" + generalFunc.getMemberId());
            data_trip_ada.put("iUserId", getIntent().getStringExtra("iUserId"));
        } else {
        ...
        getDetails();
        if (getIntent().hasExtra("iBiddingPostId")) {
            dbRef = FirebaseDatabase.getInstance().getReference().child(CommonUtilities.BUCKET_NAME + "-ServiceChat").child(data_trip_ada.get("iBiddingPostId") + "-Bid");
        } else {
    ...
    public void getDetails() {
        ...
        if (getIntent().hasExtra("iBiddingPostId")) {
            ...
            parameters.put("iUserId", getIntent().getStringExtra("iUserId"));
        } else {
            parameters.put("iTripId", getIntent().getStringExtra("iTripId"));
        }

        ApiHandler.execute(mContext, parameters,
                responseString -> {
    ...
    public void show() {
        ...
        msgbtn.setOnClickListener(view -> {
            ...
            if (Utils.checkText(input) && Utils.getText(input).length() > 0) {
                ...
                if (getIntent().hasExtra("iBiddingPostId")) {
                    dataMap.put("iBiddingPostId", data_trip_ada.get("iBiddingPostId"));
                    dataMap.put("iUserId", data_trip_ada.get("iUserId"));
                } else {
